...
* The `/exapi/v1/brokerInfo` `rateLimits` array contains objects related to the broker's `REQUEST_WEIGHT` and `ORDER` rate limits.
...
### SIGNED Endpoint Examples for POST /exapi/v1/order
...
```shell
GET /exapi/quote/v1/ticker/24hr
```
...
```shell
GET /exapi/quote/v1/ticker/price
```
...
```shell
GET /exapi/quote/v1/ticker/bookTicker
```
...
```shell
POST /exapi/v1/order  (HMAC SHA256)
```
...
```shell
POST /exapi/v1/order/test (HMAC SHA256)
```
...
```shell
GET /exapi/v1/order (HMAC SHA256)
```
...
```shell
DELETE /exapi/v1/order  (HMAC SHA256)
```
...
```shell
GET /exapi/v1/openOrders  (HMAC SHA256)
```
...
```shell
GET /exapi/v1/historyOrders (HMAC SHA256)
```
...
```shell
GET /exapi/v1/account (HMAC SHA256)
```
...
```shell
GET /exapi/v1/myTrades  (HMAC SHA256)
```
...
```shell
GET /exapi/v1/depositOrders  (HMAC SHA256)
```
...
```shell
DELETE /exapi/v1/userDataStream
```
...
```shell
POST /exapi/v1/subAccount/query
```
